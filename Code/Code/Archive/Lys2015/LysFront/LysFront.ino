...
void pin_update(){
  ...
  // Fan
  if(reg[AIRFAN_STATE]==1){
	digitalWrite(AIRFAN,1);
  }
  else{
	digitalWrite(AIRFAN,0);
  }	
  // Window Swiper
  if(reg[WSVIPER_STATE]==1){
	analogWrite(WSVIPER,128);
        //digitalWrite(WSVIPER,1);
  }
  else{
      analogWrite(WSVIPER,0);
      //digitalWrite(WSVIPER,0);
  }	
}

...
//This method sends data to the rio.
void rs232_sync(void) {
	// Send then receive
	// 2 byte sync
	// 1 byte Length
	...
	rs232_transmit(RIO_REGSIZE);				//Length
	for(unsigned char i=0;i<RIO_REGSIZE;i++)
	{
		rs232_transmit(rio_tx[i]);				//data
	}
...
*/
/* 2016
Wow it can be funny to read your 1y+ old code comments... /Kim
btw. this method receives data from the rio. (Called from a method catching the interrupt)
*/
unsigned char rs232_sync_f_rio(unsigned char datain){
	static volatile unsigned char receive_state = 0;
	static uint8_t sizeofpac = 0;
	...
	switch (receive_state){
		...
		if(regcount == sizeofpac){
			...
			//if checksum exist this should be moved to another receive state.
			for(unsigned char jj = 0; jj < sizeofpac ;jj++){

...
void Adafruit_TFTLCD::init(void) {
  ...
  RD_IDLE;
  CD_DATA;
  digitalWrite(LCD_RESET, HIGH); // Reset line
...
void Adafruit_TFTLCD::begin(uint16_t id) {
  ...
  if((id == 0x9325) || (id == 0x9328)) {
    ...
    // 9481
	Serial.println("Begin reached for 9481");
    driver = ID_9341;
    CS_ACTIVE;
	...
	Serial.println("Begin finished for 9481");
    return;
...
uint16_t Adafruit_TFTLCD::readID(void) {
  ...
  Serial.print("Found id = "); Serial.println(id, HEX);
